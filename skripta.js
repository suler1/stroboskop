window.addEventListener('load', function() {
	...
	//Odstrani barve
	var pocistiBarve = function(event) {
		var barve = document.querySelector("#barve");
		barve.innerHTML = "";
		console.log("Do sem deluje");
	}
	
	document.querySelector("#odstraniBarve")
		.addEventListener('click', pocistiBarve);
	//Stroboskop
	var vrednosti = [];
	var minCas = 0;
	var maxCas = 0;
	...
	var stop = function(event) {
		...
		s.innerHTML = "Zaženi stroboskop";
		s.addEventListener('click', zagon);
		s.removeEventListener('click', stop);
	}
